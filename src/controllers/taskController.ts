...
class TaskController {
    async createTask (req: AuthRequest, res: Response){
        ...
        try {
            const task = await taskService.createTask(title, description, dueDate, position, listId, boardId, userId);
    ...
    async getTasksByList (req: AuthRequest, res: Response){
    ...
    async updateTask (req: AuthRequest, res: Response){
        ...
        try {
            const task = await taskService.updateTask(taskId, data, userId);
            if (!task) {
                res.status(404).json({message: "Task not found"});
                return;
            }
    ...
    async moveTask (req: AuthRequest, res: Response){
        ...
        try {
            const task = await taskService.moveTask(taskId, newPosition, newListId, userId);
            if (!task) {
                res.status(404).json({message: "Task not found"});
                return;
            }
