...
class ListController {
   async createList (req: AuthRequest, res: Response){
        ...
        try {
            const newList = await listService.createList(title, position, boardId, userId);
   ...
   async getListByBoard (req: AuthRequest, res: Response) {
   ...
   async getListById (req: AuthRequest, res: Response) {
    ...
    async updateList (req: AuthRequest, res: Response) {
          ...
          try {
                const updatedList = await listService.updateList(listId, data, userId);
                if (!updatedList) {
                 res.status(404).json({message: "List not found"});
                 return;
                }
